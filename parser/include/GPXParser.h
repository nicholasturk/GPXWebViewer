...
// Represents a generic GPX element/XML node - i.e. some sort of an additinal
// piece of data,
...
typedef struct {
    ...
    // GPXData value.  We use a C99 flexible array member, which we will discuss
    // in class. Must not be an empty string
    char value[];
} GPXData;
...
typedef struct {
    ...
    // Additional waypoint data - i.e. children of the GPX waypoint other than
    // <name>. We will assume that all waypoint children have no children of
    // their own This can be elevation, time, etc.. Note that while the element
    // <name> can be a child of the waypoint node, the name already has its own
    // dedicated filed in the Waypoint sruct - so do not place the name in this
    // list All objects in the list will be of type GPXData.  It must not be
...
typedef struct {
    ...
    // All objects in the list will be of type Waypoint.  It must not be NULL.
    // It may be empty.
    List* waypoints;
    ...
    // This can be comment, description, etc.. Note that while the element
    // <name> can be a child of the route node, the name already has its own
    // dedicated filed in the Waypoint sruct - so do not place the name in this
    // list All objects in the list will be of type GPXData.  It must not be
    // NULL.  It may be empty.
    List* otherData;
...
typedef struct {
    ...
    // All objects in the list will be of type TrackSegment.  It must not be
    // NULL.  It may be empty.
    List* segments;
    ...
    // We will assume that all waypoint children have no children of their own
    // This can be comment, description, etc.. Note that while the element
    // <name> can be a child of the track node, the name already has its own
    // dedicated filed in the Waypoint sruct - so do not place the name in this
    // list All objects in the list will be of type GPXData.  It must not be
    // NULL.  It may be empty.
    List* otherData;
...
typedef struct {
    // Namespace associated with our GPX doc.  Must not be an empty string.
    // While a real GPX doc might have multiple namespaces associated with it,
    ...
    // All objects in the list will be of type Route.  It must not be NULL.  It
    // may be empty.
    List* routes;

    // Tracks in the GPX file
    // All objects in the list will be of type Track.  It must not be NULL.  It
    // may be empty.
    List* tracks;
} GPXdoc;
 ...
 *@post GPX has not been modified in any way, and a string representing the GPX
...
/* For the five "get..." functions below, return the count of specified entities
from the file. They all share the same format and only differ in what they have
to count.
...
// Function that returns a waypoint with the given name.  If more than one
// exists, return the first one. Return NULL if the waypoint does not exist
Waypoint* getWaypoint(const GPXdoc* doc, char* name);
// Function that returns a track with the given name.  If more than one exists,
// return the first one. Return NULL if the track does not exist
Track* getTrack(const GPXdoc* doc, char* name);
// Function that returns a route with the given name.  If more than one exists,
// return the first one. Return NULL if the route does not exist
...
/** Function to create an GPX object based on the contents of an GPX file.
 * This function must validate the XML tree generated by libxml against a GPX
schema file
...
/** Function to validating an existing a GPXobject object against a GPX schema
 file
 *@pre
...
/** Function that returns the number routes with the specified length, using the
 *provided tolerance to compare route lengths
...
/** Function that returns the number tracks with the specified length, using the
 *provided tolerance to compare track lengths
 *@pre GPXdoc object exists, is not null
 *@post GPXdoc object exists, is not null, has not been modified
 ...
 *@param delta - the tolerance used for comparing distances between start and
 *end points
 **/
...
/** Function that returns all routes between the specified start and end
 *locations
 *@pre GPXdoc object exists, is not null
 *@post GPXdoc object exists, is not null, has not been modified
...
/** Function that returns all Tracks between the specified start and end
 *locations
 *@pre GPXdoc object exists, is not null
 *@post GPXdoc object exists, is not null, has not been modified
...
char* GPXDataToJSON(const GPXData* od);
...
List* getTrackList(const GPXdoc* doc);
List* getRouteList(const GPXdoc* doc);
...
int addRouteWrapper(GPXdoc* doc, char* fileName, char* routeName,
                    float latitudes[], float longitudes[], int numPoints);
